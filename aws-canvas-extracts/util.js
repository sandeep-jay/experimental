...
var downloadFiles = module.exports.downloadFiles = function(path, files, callback) {
  ...
  async.eachSeries(files, function(file, done) {
    ...
    console.info({'file': filename}, 'Starting Canvas data file download');

    // Don't download the file if it has already been downloaded
    fs.exists(path, function(exists) {
      if (exists) {
        console.info({'file': file}, 'Skipping Canvas data file download. File already exists');
        return done();
      } else {
        var start = Date.now();
        var gunzip = zlib.createGunzip();
        gunzip.on('error', function(err) {
          console.error({'err': err, 'file': filename}, 'Unable to gunzip Canvas data file stream');
          return done();
        });
        var fileStream = request(file.url).pipe(gunzip).pipe(fs.createReadStream(filename))
        ...
        .on('finish', function() {
          var end = Date.now();
          console.info({'file': filename, 'duration': (end - start)}, 'Finished Canvas data file download');
          return done();
        });
